...
def iniciar_servidor():
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as server:
        ...
        while True:
            ...
            thread = threading.Thread( #Quando o cliente entra a thread passa a tratar dele, enquanto espera outra
                target=atender_cliente,
                args=(conn, addr), #Parametros, quando passa parametros em threads use args=()
                daemon=True
            )
            thread.start()
